...
namespace MockMetrics.Eating
{
    public static class Eater
    {
        ...
        static Eater()
        {
            ...
            _eaters[typeof (IExpressionStatement)] = new ExpressionStatementEater();
            _eaters[typeof (IInvocationExpression)] = new InvocationEater();
            _eaters[typeof (ICSharpLiteralExpression)] = new LiteralExpressionEater();
            _eaters[typeof (IObjectCreationExpression)] = new ObjectCreationEater();
            _eaters[typeof (IReferenceExpression)] = new ReferenceEater();
        ...
        private static IStatementEater<T> GetStatementEater<T>() where T : ICSharpStatement
        {
            return _eaters[typeof (T)] as IStatementEater<T>;
        }

        private static IExpressionEater<T> GetExpressionEater<T>() where T : ICSharpExpression
        {
            return _eaters[typeof (T)] as IExpressionEater<T>;
        }

        public static ExpressionKind Eat(Snapshot snapshot, IMethodDeclaration test, ICSharpExpression expression)
        {
            if (expression == null)
                throw new ArgumentNullException("expression");

            return EatExpression(snapshot, test, expression);
        }

        public static void Eat(Snapshot snapshot, IMethodDeclaration test, ICSharpStatement statement)
        {
            if (statement == null)
                throw new ArgumentNullException("statement");

            EatStatement(snapshot, test, statement);
        }

        private static ExpressionKind EatExpression(Snapshot snapshot, IMethodDeclaration test,
        ...
        private static void EatStatement(Snapshot snapshot, IMethodDeclaration test, ICSharpStatement statement)
        {

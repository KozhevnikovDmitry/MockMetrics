...
{
    public class UnemployedPaymentTaskXmlImporter : TaskXmlImporter
    {
        private static readonly XNamespace Namespace =
            @"urn://x-artefacts-it-ru/dob/state-services/krsk/UnemployedPayment/1.5";

        private static readonly List<ContentXsdMatch> UnemployedPaymentMatches = new List<ContentXsdMatch>
            {
                new ContentXsdMatch {ContentUri = "trud/services/unemployedPayment", RootName = Namespace + "UnemployedPayment"}
            };

        
        public UnemployedPaymentTaskXmlImporter(IContentPolicy contentPolicy, ITaskPolicy taskPolicy, IDictionaryManager dictionaryManager)
            : base(UnemployedPaymentMatches, contentPolicy, dictionaryManager, taskPolicy)
        {
        }

...
{
    public class MedicalAdcTaskXmlImporter : TaskXmlImporter
    {
        private static readonly XNamespace Namespace =
            @"urn://x-artefacts-it-ru/dob/state-services/krsk/MedicalADC/1.5";

        private static readonly List<ContentXsdMatch> MedicalADCMatches = new List<ContentXsdMatch>
            {
                new ContentXsdMatch {ContentUri = "mz/cert/medicalADC", RootName = Namespace + "MedicalADC"}
            };

        public MedicalAdcTaskXmlImporter(IContentPolicy contentPolicy, ITaskPolicy taskPolicy, IDictionaryManager dictionaryManager)
            : base(MedicalADCMatches, contentPolicy, dictionaryManager, taskPolicy)
        {
        }

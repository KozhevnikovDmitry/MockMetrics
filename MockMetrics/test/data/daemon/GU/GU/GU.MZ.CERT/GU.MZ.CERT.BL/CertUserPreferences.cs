...
using GU.MZ.CERT.BL.Search;

namespace GU.MZ.CERT.BL
{
    /// <summary>
    /// Класс, предназначенный для управления пользовательскими настройками приложения Аттестация.
    /// </summary>
    public class CertUserPreferences : IUserPreferences
    {
        /// <summary>
        /// Класс, предназначенный для управления пользовательскими настройками приложения Аттестация.
        /// </summary>
        public CertUserPreferences()
        {
            this._configDirectoryPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "GU.MZ.CERT");
            this._сonfigurationPath = Path.Combine(this._configDirectoryPath, "CertConfig.xml");
        ...
        public void LoadPresets()
        {
            try
            {
                this.SearchPresetSpecContainer = new CertSearchPresetSpecContainer();
        ...
        /// Контейнер настроек.
        /// </summary>
        private CertUserPreferencesContainer _preferencesContainer;
        ...
        {
            this._preferencesContainer = new CertUserPreferencesContainer();
        }
            ...
            try
            {
                if (File.Exists(this._сonfigurationPath))
                {
                    using (var stream = new FileStream(this._сonfigurationPath, FileMode.Open))
                    {
                        ...
                        this._preferencesContainer = (CertUserPreferencesContainer)sf.Deserialize(stream);
                    }
                }
                else
                {
    ...
    /// <summary>
    /// Класс, предназначенный для хранения пользовательских настроек приложения Аттестация.
    /// </summary>
    [Serializable]
    internal class CertUserPreferencesContainer
    {
        /// <summary>
        /// Класс, предназначенный для хранения пользовательских настроек приложения Аттестация.
        /// </summary>
        public CertUserPreferencesContainer()
        {

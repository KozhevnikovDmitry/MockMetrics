...
namespace GU.MZ.DS.ViewModel
{
    ...
    public class MainVM : NotificationObject
    {
        ...
        public MainVM()
        {
            ...
            var licenseModuleVm = new DrugSupplyModuleVM();

            this.SetInteractionBehavior(taskModuleVm, licenseModuleVm);

            this.ModuleVmList = new List<IWorkspaceVM> 
            { 
                new ToolVM { View = new DrugSupplyModuleView { DataContext = licenseModuleVm } }, 
                new ToolVM { View = new TaskModuleView { DataContext = taskModuleVm } }
            };

            this.ModuleSelectorVmList = new List<ModuleSelectorVM>
            {
                new ModuleSelectorVM { DisplayName = "Модуль лекарственное обеспечение", IconPath = "/GU.UI;component/Resources/Icons/drugsupplay.png" },
        ...
        /// <param name="drugSupplyModuleVM">Модуль лицензирования</param>
        /// <remarks>
        /// В методе задаётся то, каким образом модули могут обмениваться событиями на открытие вкладок AvalonDock. 
        /// </remarks>
        private void SetInteractionBehavior(TaskModuleVM taskModuleVM, DrugSupplyModuleVM drugSupplyModuleVM)
        {
            taskModuleVM.AvalonInteractor.OpenEditableDockable += (s, e) => this.OpenEditableDockableOnModule(s, e, drugSupplyModuleVM);
            drugSupplyModuleVM.AvalonInteractor.OpenEditableDockable += (s, e) => this.OpenEditableDockableOnModule(s, e, taskModuleVM);
        }

...
{
    [TestFixture]
    public class LicenseRequisitesDataMapperTests : BaseTestFixture
    {
        ...
        {
            // Arrange
            var requisites = Mock.Of<LicenseRequisites>();
            var db = Mock.Of<IDomainDbManager>(t => t.RetrieveDomainObject<LicenseRequisites>(1) == requisites);
            var mapper = new LicenseRequisitesDataMapper(new StubDbCtx(db));
        ...
        {
            // Arrange
            var requisites = Mock.Of<LicenseRequisites>(t => t.AddressId == 2);
        ...
        {
            // Arrange
            var requisites = Mock.Of<LicenseRequisites>(t => t.JurRequisitesId == 2);
        ...
        {
            // Arrange
            var requisites = Mock.Of<LicenseRequisites>(t => t.IndRequisitesId == 2);
        ...
        public void SaveRequisitesTest()
        {
            ...
            var address = Mock.Of<Address>();
            var jurReqs = Mock.Of<JurRequisites>();
            var requisites = Mock.Of<LicenseRequisites>(t => t.Address == address && t.JurRequisites == jurReqs);
            var db = new Mock<IDomainDbManager>();
            var mapper = new LicenseRequisitesDataMapper(new StubDbCtx(db.Object));

            // Act
            var result = mapper.Save(Mock.Of<LicenseRequisites>(t => t.Clone() == requisites));
        ...
        public void SaveIndividualTest()
        {
            ...
            var requisites = Mock.Of<LicenseRequisites>(t => t.Address == address && t.IndRequisites == indReqs);
            var db = new Mock<IDomainDbManager>();
            var mapper = new LicenseRequisitesDataMapper(new StubDbCtx(db.Object));

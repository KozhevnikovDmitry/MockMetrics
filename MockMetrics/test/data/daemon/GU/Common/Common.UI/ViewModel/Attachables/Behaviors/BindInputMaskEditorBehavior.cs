...
{
    /// <summary>
    /// Поведение для <c>MaskEditor</c> ввода, предназначенное для того, чтобы производить явный <c>UpdateSourceTrigger</c> по определённым событиям.
    /// </summary>
    /// <remarks>
    /// Поведение предназначено на <c>MaskEditor</c>'ов диалоговых окон, в особенности для окон с <c>ToolBar</c>.
    ...
    /// </remarks>
    public class BindInputMaskEditorBehavior : Behavior<MaskEditor>
    {
        ...
        {
            BindingExpression be = AssociatedObject.GetBindingExpression(MaskInputBase.ValueProperty);
            if (be != null)
            {
                be.UpdateSource();
            }

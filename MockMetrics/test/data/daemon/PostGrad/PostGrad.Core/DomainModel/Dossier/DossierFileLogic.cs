...
using PostGrad.Core.DomainModel.Inspect;
using PostGrad.Core.DomainModel.MzOrder;
using PostGrad.Core.DomainModel.Notifying;
using PostGrad.Core.DomainModel.Violation;
...
namespace PostGrad.Core.DomainModel.Dossier
{
    ...
    public partial class DossierFile
    {
        ...
        [NoInstance]
        [CloneIgnore]
        public virtual bool IsGrantingPositve
        {
            get
            {
                if (TaskState == TaskStatusType.Ready)
                {
                    return true;
                }

                if (TaskState == TaskStatusType.Rejected)
                {
                    return false;
                }
